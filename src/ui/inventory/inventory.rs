        ...
        egui::SidePanel::left("inventory_menu").show(ui.ctx(), |ui|{
            ...
            .show(ui, |ui| {
                ui.vertical(|ui| {
                    ...
                    if add_items.clicked() {vm.slots[vm.index].inventory_vm.current_route = InventoryRoute::Add}
                    if browse_items.clicked() {vm.slots[vm.index].inventory_vm.current_route = InventoryRoute::Browse}
