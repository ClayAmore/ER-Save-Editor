...
pub fn browse_inventory(ui: &mut Ui, vm: &mut ViewModel) {
    ...
    ui.columns(2, |uis| {
        ...
        let storage_button =
            uis[1].add_sized([100., 40.], egui::widgets::Button::new("Storage Box"));

        if equipped_button.clicked() {
            inventory_vm.at_storage_box = false;
        };
        if storage_button.clicked() {
            inventory_vm.at_storage_box = true;
        };

        if inventory_vm.at_storage_box {
            storage_button.highlight();
        } else {
            equipped_button.highlight();
        }
    });
    ...
    ui.columns(6, |uis| {
        ...
        if key_items.clicked() {
            inventory_vm.current_type_route = InventoryTypeRoute::KeyItems
        }
        if weapons.clicked() {
    ...
    let current_inventory_list = match inventory_vm.current_type_route {
        InventoryTypeRoute::CommonItems => {
            &inventory_vm.storage[inventory_vm.at_storage_box as usize].filtered_items
        }
        InventoryTypeRoute::KeyItems => {
            &inventory_vm.storage[inventory_vm.at_storage_box as usize].filtered_key_items
        }
        InventoryTypeRoute::Weapons => {
            &inventory_vm.storage[inventory_vm.at_storage_box as usize].filtered_weapons
        }
        InventoryTypeRoute::Armors => {
            &inventory_vm.storage[inventory_vm.at_storage_box as usize].filtered_armors
        }
        InventoryTypeRoute::AshOfWar => {
            &inventory_vm.storage[inventory_vm.at_storage_box as usize].filtered_aows
        }
        InventoryTypeRoute::Talismans => {
            &inventory_vm.storage[inventory_vm.at_storage_box as usize].filtered_accessories
        }
    };
