use crate::{
    db::{
        accessory_name::accessory_name::ACCESSORY_NAME, aow_name::aow_name::AOW_NAME, aows::aows,
        armor_name::armor_name::ARMOR_NAME, armors::armor_sets, item_name::item_name::ITEM_NAME,
        items::items, talismans::talismans, weapon_name::weapon_name::WEAPON_NAME,
        weapons::weapons,
    },
    ui::custom::checkbox::checkbox::{three_states_checkbox, State},
    util::regulation::Regulation,
    vm::{
        inventory::{InventoryTypeRoute, InventoryViewModel},
        regulation::regulation_view_model::{GoodsType, RegulationViewModel, WepType},
        vm::vm::ViewModel,
    },
};
use eframe::{
    egui::{self, Layout, Ui, Vec2},
    epaint::Color32,
};
    ...
    egui::TopBottomPanel::top("top_panel").show(ui.ctx(), |ui| {
        ...
        ui.columns(2, |uis| {
            ...
            if bulk_button.clicked() {
                inventory_vm.at_single_items = false;
            };

            if inventory_vm.at_single_items {
                single_button.highlight();
            } else {
                bulk_button.highlight();
            }
        });
        ...
        ui.columns(5, |uis| {
            let common_items =
                uis[0].add_sized([uis[0].available_width(), 40.], egui::Button::new("Items"));
            let weapons = uis[1].add_sized(
                [uis[1].available_width(), 40.],
                egui::Button::new("Weapons"),
            );
            let armors =
                uis[2].add_sized([uis[2].available_width(), 40.], egui::Button::new("Armors"));
            let ashofwar = uis[3].add_sized(
                [uis[3].available_width(), 40.],
                egui::Button::new("Ash of War"),
            );
            let talismans = uis[4].add_sized(
                [uis[4].available_width(), 40.],
                egui::Button::new("Talismans"),
            );

            if common_items.clicked() {
                // Update route for single item add
                inventory_vm.current_type_route = InventoryTypeRoute::CommonItems;
                regulation_vm.selected_item = Default::default();
            ...
            if weapons.clicked() {
                // Update route for single item add
                inventory_vm.current_type_route = InventoryTypeRoute::Weapons;
                regulation_vm.selected_item = Default::default();
            ...
            if armors.clicked() {
                // Update route for single item add
                inventory_vm.current_type_route = InventoryTypeRoute::Armors;
                regulation_vm.selected_item = Default::default();
            ...
            if ashofwar.clicked() {
                // Update route for single item add
                inventory_vm.current_type_route = InventoryTypeRoute::AshOfWar;
                regulation_vm.selected_item = Default::default();
            ...
            if talismans.clicked() {
                // Update route for single item add
                inventory_vm.current_type_route = InventoryTypeRoute::Talismans;
                regulation_vm.selected_item = Default::default();
            ...
            match inventory_vm.at_single_items {
                true => match inventory_vm.current_type_route {
                    InventoryTypeRoute::CommonItems => {
                        common_items.highlight();
                    }
                ...
                false => match inventory_vm.current_bulk_type_route {
                    InventoryTypeRoute::CommonItems => {
                        common_items.highlight();
                    }
                    InventoryTypeRoute::KeyItems => {}
            ...
            InventoryTypeRoute::CommonItems => {
                egui::ScrollArea::vertical()
                    ...
                    .show_rows(
                        ui,
                        row_height,
                        regulation_vm.filtered_goods.len(),
                        |ui, row_range| {
                            for i in row_range {
                                ...
                                if regulation_vm.selected_item.id == item.id {
                                    text = egui::RichText::new(format!("{}", item.name))
                                        .strong()
                                        .heading();
                                }
            ...
            InventoryTypeRoute::Weapons => {
                egui::ScrollArea::vertical()
                    ...
                    .show_rows(
                        ui,
                        row_height,
                        regulation_vm.filtered_weapons.len(),
                        |ui, row_range| {
                            for i in row_range {
                                let mut text = egui::RichText::new(format!(
                                    "{}",
                                    &regulation_vm.filtered_weapons[i].name
                                ));
                                if regulation_vm.selected_item.id
                                    == regulation_vm.filtered_weapons[i].id
                                {
                                ...
                                ui.with_layout(Layout::left_to_right(egui::Align::Min), |ui| {
                                    if ui
                                        ...
                                        .clicked()
                                    {
                                        regulation_vm.selected_item =
                                            regulation_vm.filtered_weapons[i].clone();
            ...
            InventoryTypeRoute::Armors => {
                egui::ScrollArea::vertical()
                    ...
                    .show_rows(
                        ui,
                        row_height,
                        regulation_vm.filtered_protectors.len(),
                        |ui, row_range| {
                            for i in row_range {
            ...
            InventoryTypeRoute::AshOfWar => {
                egui::ScrollArea::vertical()
                    ...
                    .show_rows(
                        ui,
                        row_height,
                        regulation_vm.filtered_gems.len(),
                        |ui, row_range| {
                            for i in row_range {
            ...
            InventoryTypeRoute::Talismans => {
                egui::ScrollArea::vertical()
                    ...
                    .show_rows(
                        ui,
                        row_height,
                        regulation_vm.filtered_accessories.len(),
                        |ui, row_range| {
                            for i in row_range {
