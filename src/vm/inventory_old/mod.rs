    ...
    pub fn filter(&mut self) {
        for inventory_storage in &mut self.storage {
            ...
            inventory_storage.filtered_weapons.sort_by(|a, b| {
                ...
                let distance_a = sorensen_dice(
                    &a.item_name.to_lowercase(),
                    &self.filter_text.to_lowercase(),
                );
                let distance_b = sorensen_dice(
...
/*
    let mut weaponId: u32 = 3180725;

    let uVar:u32 = 0xffffffff;

    if weaponId & 0xf0000000 == 0 {
        let mut uVar2: u32 = 0xffffffff;

        if weaponId & uVar != uVar {
            uVar2 = weaponId & uVar;
            println!("uVar2: {:x}", uVar2);
        }

        let mut uVar5: u32 = 0xffffffff;

        if weaponId >> 0x1c != 0xf {
            uVar5 = weaponId >> 0x1c;
            println!("uVar5: {:x}", uVar5);
        }

        weaponId = (uVar2 / 10000) * 10000 & 0xfffffff | uVar5 << 0x1c;
