...
pub mod equipment_view_model {
    ...
    impl EquipmentViewModel {
        pub fn from_save(slot: &SaveSlot) -> Self {
            ...
            let gaitem_map: HashMap<u32, GaItem> = slot
                .ga_items
                .iter()
            ...
            equipment_vm
                ...
                .for_each(|(index, weapon)| {
                    Self::weapon(
                        ...
                        weapon,
                        &gaitem_map,
                        &slot.chr_asm2.left_hand_armaments[index],
                    )
                });
            ...
            equipment_vm
                ...
                .for_each(|(index, weapon)| {
                    Self::weapon(
                        ...
                        weapon,
                        &gaitem_map,
                        &slot.chr_asm2.right_hand_armaments[index],
                    )
                });
            ...
            equipment_vm
                ...
                .iter_mut()
                .enumerate()
                .for_each(|(talisman_index, talisman)| {
            ...
            equipment_vm
                ...
                .iter_mut()
                .enumerate()
                .for_each(|(qs_index, qs)| {
                    Self::item(
                        slot,
                        qs,
                        slot.equip_item_data.quick_slot_items[qs_index].item_id,
                    )
                });
        ...
        fn weapon(
            slot: &SaveSlot,
            weapon: &mut EquipmentItemViewModel,
            gaitem_map: &HashMap<u32, GaItem>,
            gaitem_handle: &u32,
        ) {
            ...
            weapon.name = Self::name_or_empty(
        ...
        fn projectile(
            slot: &SaveSlot,
            projectile: &mut EquipmentItemViewModel,
            gaitem_map: &HashMap<u32, GaItem>,
            gaitem_handle: &u32,
        ) {
            projectile.gaitem_handle = *gaitem_handle;
            projectile.id = if gaitem_map.get(gaitem_handle).is_some() {
                gaitem_map[gaitem_handle].item_id
            } else {
                0
            };
            projectile.equip_index = Self::equip_index(slot, *gaitem_handle);
            projectile.name =
                Self::name_or_empty(Regulation::equip_weapon_params_map(), projectile.id);
        }
        ...
        fn armor(
            ...
            armor.id = if gaitem_map[gaitem_handle].item_id != 0 {
                gaitem_map[gaitem_handle].item_id ^ 0x10000000
            } else {
                0
            };
        ...
        fn talisman(slot: &SaveSlot, talisman: &mut EquipmentItemViewModel, gaitem_handle: u32) {
            ...
            talisman.id = if gaitem_handle != 0 {
                gaitem_handle ^ 0xA0000000
            } else {
                0
            };
            talisman.equip_index = Self::equip_index(slot, gaitem_handle);
            talisman.name =
                Self::name_or_empty(Regulation::equip_accessory_param_map(), talisman.id);
        }

        fn item(slot: &SaveSlot, item: &mut EquipmentItemViewModel, gaitem_handle: u32) {
            item.gaitem_handle = gaitem_handle;
            item.id = if gaitem_handle != 0 {
                gaitem_handle ^ InventoryGaitemType::ITEM as u32
            } else {
                0
            };
        ...
        fn name_or_empty<T>(map: &HashMap<u32, Row<T>>, id: u32) -> String
        where
            T: Default + Clone,
        {
            let res = map.get(&id);
