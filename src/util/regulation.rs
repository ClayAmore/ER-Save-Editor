...
use crate::{
    ...
    util::{
        param_structs::{
            EQUIP_PARAM_ACCESSORY_ST, EQUIP_PARAM_GEM_ST, EQUIP_PARAM_GOODS_ST,
            EQUIP_PARAM_PROTECTOR_ST, EQUIP_PARAM_WEAPON_ST,
        },
        params::params::{Row, PARAM},
    },
};

use super::{bnd4::bnd4::BND4, params::params::Param};

pub static PARAMS: Lazy<RwLock<HashMap<Param, Vec<u8>>>> =
...
impl Regulation {
    pub fn init_params(save: &Save) {
        *PARAMS.write().unwrap() =
            Regulation::params_from_regulation(save.save_type.get_regulation()).unwrap();
    }

    pub fn equip_accessory_param_map() -> &'static HashMap<u32, Row<EQUIP_PARAM_ACCESSORY_ST>> {
        static ACCESSORY_PARAM_MAP: OnceCell<HashMap<u32, Row<EQUIP_PARAM_ACCESSORY_ST>>> =
            OnceCell::new();
        ACCESSORY_PARAM_MAP.get_or_init(|| {
            let mut map =
                Self::get_param_map::<EQUIP_PARAM_ACCESSORY_ST>(&Param::EquipParamAccessory);
    ...
    pub fn equip_protectors_param_map() -> &'static HashMap<u32, Row<EQUIP_PARAM_PROTECTOR_ST>> {
        static PROTECTOR_PARAM_MAP: OnceCell<HashMap<u32, Row<EQUIP_PARAM_PROTECTOR_ST>>> =
            OnceCell::new();
        PROTECTOR_PARAM_MAP.get_or_init(|| {
            let mut map =
                Self::get_param_map::<EQUIP_PARAM_PROTECTOR_ST>(&Param::EquipParamProtector);
    ...
    pub fn equip_weapon_params_map() -> &'static HashMap<u32, Row<EQUIP_PARAM_WEAPON_ST>> {
        static WEAPON_PARAM_MAP: OnceCell<HashMap<u32, Row<EQUIP_PARAM_WEAPON_ST>>> =
            OnceCell::new();
    ...
    pub fn params_from_regulation(bytes: &[u8]) -> Result<HashMap<Param, Vec<u8>>, Error> {
        ...
        for file in &res.files {
            let file_name = &file
                ...
                .split(".")
                .nth(0)
                .expect("Could not locate file name without extension");
