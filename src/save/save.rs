pub mod save {
    use crate::{
        ...
        save::{
            common::{
                save_slot::{
                    EquipInventoryData, EquipProjectileData, GaItem, GaItemData, SaveSlot,
                },
    ...
    const REGULATION_MD5_CHECKSUM: [u8; 0x10] = [
        0x9D, 0xE4, 0x83, 0x80, 0x78, 0xB2, 0x28, 0x9D, 0x83, 0x8D, 0x28, 0x7A, 0x24, 0x31, 0xE6,
        0x45,
    ];
    ...
    impl SaveType {
            ...
            match self {
                ...
                SaveType::PC(pc_save) => {
                    pc_save.save_slots[index]
                        ...
                        .player_game_data
                        .character_name
                        .copy_from_slice(&character_name);
                    pc_save.user_data_10.profile_summary[index]
                        .character_name
        ...
        pub fn set_character_souls(&mut self, index: usize, souls: u32) {
            match self {
                SaveType::Unknown => panic!("Why are we here?"),
                SaveType::PC(pc_save) => {
                    let orgininal_soulsmemory = pc_save.save_slots[index]
                        .save_slot
                        .player_game_data
                        .soulsmemory;
                    let orgininal_souls = pc_save.save_slots[index]
                        .save_slot
                        .player_game_data
                        .soulsmemory;
                    ...
                    if souls > orgininal_souls {
                        pc_save.save_slots[index]
                            .save_slot
                            .player_game_data
                            .soulsmemory = orgininal_soulsmemory + souls;
                    }
                }
                SaveType::PlayStation(ps_save) => {
                    let orgininal_soulsmemory =
        ...
        pub fn set_character_event_flag(
            &mut self,
            index: usize,
            offset: usize,
            bit_pos: u8,
            ...
            match self {
                ...
                SaveType::PC(pc_save) => {
                    ...
                    pc_save.save_slots[index].save_slot.event_flags.flags[offset] =
        ...
        pub fn add_region(&mut self, index: usize, region_id: u32) {
            match self {
                SaveType::Unknown => panic!("Why are we here?"),
                SaveType::PC(pc_save) => {
                    let res = pc_save.save_slots[index]
                        .save_slot
                        .regions
                        .unlocked_regions
                        .iter()
                    ...
                    match res {
                        ...
                        None => {
                            ...
                            pc_save.save_slots[index]
                                .save_slot
                                .regions
                                .unlocked_regions_count = pc_save.save_slots[index]
                                .save_slot
                                .regions
                                .unlocked_regions_count
                                + 1;
                        }
                    }
                }
                SaveType::PlayStation(ps_save) => {
                    let res = ps_save.save_slots[index]
                        .regions
                        .unlocked_regions
                        .iter()
                        .position(|r| *r == region_id);
                    match res {
                        ...
                        None => {
                            ps_save.save_slots[index]
                                ...
                                .unlocked_regions
                                .push(region_id);
                            ps_save.save_slots[index].regions.unlocked_regions_count =
        ...
        pub fn set_profile_summary(&mut self, index: usize, profile_summary: ProfileSummary) {
            match self {
                SaveType::Unknown => panic!("Why are we here?"),
                SaveType::PC(pc_save) => {
                    pc_save.user_data_10.profile_summary[index] = profile_summary
                }
                SaveType::PlayStation(ps_save) => {
        ...
        pub fn set_gaitem_map(&mut self, index: usize, ga_items: Vec<GaItem>) {
            match self {
                SaveType::Unknown => panic!("Why are we here?"),
                SaveType::PC(pc_save) => {
                    pc_save.save_slots[index].save_slot.ga_items = ga_items;
                }
                SaveType::PlayStation(ps_save) => {
        ...
        pub fn set_storage_box_inventory(
            &mut self,
            index: usize,
            storage_box_inventory: EquipInventoryData,
        ) {
            match self {
                SaveType::Unknown => panic!("Why are we here?"),
                SaveType::PC(pc_save) => {
                    pc_save.save_slots[index].save_slot.storage_inventory_data =
        ...
        pub fn set_quickslot_item(
            &mut self,
            slot_index: usize,
            quickslot_index: usize,
            gaitem_handle: u32,
            item_id: u32,
            equip_index: u32,
        ) {
            match self {
                SaveType::Unknown => panic!("Why are we here?"),
                SaveType::PC(pc_save) => {
                    pc_save.save_slots[slot_index]
                        .save_slot
                        .equipped_items
                        .quickitems[quickslot_index] = item_id;
                    pc_save.save_slots[slot_index]
                        .save_slot
                        .equip_item_data
                ...
                SaveType::PlayStation(ps_save) => {
                    ps_save.save_slots[slot_index].equipped_items.quickitems[quickslot_index] =
                        item_id;
                    ps_save.save_slots[slot_index]
                        .equip_item_data
                        .quick_slot_items[quickslot_index]
                        .item_id = gaitem_handle;
                    ps_save.save_slots[slot_index]
            ...
            match self {
                ...
                SaveType::PC(pc_save) => {
                    pc_save.save_slots[slot_index]
                        .save_slot
                        .equipped_items
                        .pouch[pouch_index] = item_id;
                    pc_save.save_slots[slot_index]
                        .save_slot
                        .equip_item_data
                        .pouch_items[pouch_index]
                        .item_id = gaitem_handle;
                    pc_save.save_slots[slot_index]
                        .save_slot
                        .equip_item_data
        ...
        pub fn set_left_weapon_slot(
            &mut self,
            slot_index: usize,
            weapon_slot_index: usize,
            gaitem_handle: u32,
            item_id: u32,
            equip_index: u32,
        ) {
        ...
        pub fn set_right_weapon_slot(
            ...
            match self {
                ...
                SaveType::PC(pc_save) => {
                    ...
                    profile_summary.equipment_gaitem.right_hand_armaments[weapon_slot_index] =
                        gaitem_handle;
        ...
        pub fn set_bolt_slot(
            ...
            match self {
                ...
                SaveType::PC(pc_save) => {
                    ...
                    slot.chr_asm.bolts[weapon_slot_index] =
                        if item_id == 0 { u32::MAX } else { item_id };
                    slot.chr_asm2.bolts[weapon_slot_index] = gaitem_handle;
                    slot.equipped_items.bolts[weapon_slot_index] =
                        if item_id == 0 { u32::MAX } else { item_id };
        ...
        pub fn set_equip_projectile_data(
            &mut self,
            index: usize,
            projectile_list: EquipProjectileData,
        ) {
            match self {
                SaveType::Unknown => panic!("Why are we here?"),
                SaveType::PC(pc_save) => {
                    pc_save.save_slots[index].save_slot.equip_projectile_data =
                        projectile_list.clone();
                }
                SaveType::PlayStation(ps_save) => {
